...
def insert_zipper1(idx,dataframe):
    inputs=st.text_input('Introduce the Teeth')
    inputs=inputs.split(',')
    return inputs

def insert_zipper(idx,dataframe):
    inputs=st.text_input('Introdueix the Label')
    inputs=inputs.split(',')
    return inputs
...
def zippers_model(df_not_encoded11,df, dataset,df_not_encoded, important ,predict, quartils,relative_error_mean_reduced,previa,dataset_principi,num,dataset_principi_modified,df_encoded1):
    ...
    llargades=[]
    indexs_cremalleres=[]
    caracteristicas = st.text_input("Introduce the features (separated by a comma and with CAPITAL letter): ")
    caracteristicas = caracteristicas.split(',')

    label=insert_zipper(0,df_not_encoded)
    teeth=insert_zipper1(0,df_not_encoded)

    cremalleres_filtrades=pd.DataFrame()

    for caract in caracteristicas:
        if caract in df_not_encoded['Familia'].tolist():
            families.append(caract)
        if caract in df_not_encoded['Stopers'].tolist():
            stoperss.append(caract)
        if caract in df_not_encoded['Sliders'].tolist():
            sliderss.append(caract)
        if caract in df_not_encoded['Color'].tolist():
            colors.append(caract)
        if caract in df_not_encoded['Llargada'].tolist():
            llargades.append(caract)
        
    for t in label:
        if t in df_not_encoded['Label'].tolist():
            labels.append(t)
    for j in teeth: 
        if j in df_not_encoded['Teeth'].tolist():
            teeths.append(j)


    for index, row in df_not_encoded.iterrows():
        contains_all_characteristics=True
        if str(row['Familia']) not in str(families) and families!=[]:
            contains_all_characteristics=False
        if str(row['Stopers']) not in str(stoperss) and stoperss!=[]:
            contains_all_characteristics=False
        if str(row['Sliders']) not in str(sliderss) and sliderss!=[]:
            contains_all_characteristics=False
        if str(row['Label']) not in str(labels) and labels!=[]:
            contains_all_characteristics=False
        if str(row['Teeth']) not in str(teeths) and teeths!=[]:
            contains_all_characteristics=False
        if str(row['Color']) not in str(colors) and colors!=[]:
            contains_all_characteristics=False
        if str(row['Llargada']) not in str(llargades) and llargades!=[]:
            contains_all_characteristics=False
